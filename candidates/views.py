...
@login_required#(login_url='/users/templates/users/login.html')
...
def candidate_details(request):
    ...
    object_list = Job.objects.all().order_by('-date_posted')[:6]
    data={
        'candidate_home': "active",
        'revdata':revdata,
        'object_list':object_list
    }
    return render(request, 'candidates/details.html',data)
